...
The WebSocket API provides real-time, bidirectional communication with Deskctl Bridge. Unlike HTTP polling, updates are pushed to you instantly.
...
<Callout type="success" title="Efficient by Design">
  Deskctl Bridge uses **demand-based monitoring**. No polling loops run until a client
