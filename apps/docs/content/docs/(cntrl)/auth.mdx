...
description: Secure your Deskctl API with API keys and IP whitelisting.
icon: Shield
---

<Callout type="error" title="Security Warning">
  **Authentication alone may not make Deskctl safe for the public internet.** We strongly
  recommend against exposing port `9990` via router port forwarding. Use a VPN
  (Tailscale/WireGuard) for remote access.
</Callout>

By default, Deskctl's API is open to anyone on your network. If you are on a **shared network** (like a dorm or office) or using a VPN like Tailscale, you should enable authentication to secure endpoints.
