; NSIS Installer Hooks for Deskctl Bridge

; Hook: Runs after installation completes
!macro NSIS_HOOK_POSTINSTALL
    ; Write default config.json to user's AppData
    SetShellVarContext current
    CreateDirectory "$APPDATA\com.codewithevilxd.deskctl-bridge"

    ; Only create config if it doesn't exist (preserve existing config)
    IfFileExists "$APPDATA\com.codewithevilxd.deskctl-bridge\config.json" skipConfig

    ; Write JSON config file with default values
    FileOpen $0 "$APPDATA\com.codewithevilxd.deskctl-bridge\config.json" w
    FileWrite $0 "{$\r$\n"
    ...
    ; Add firewall rule
    DetailPrint "Adding Firewall Rule for Deskctl Bridge..."
    nsExec::ExecToLog 'netsh advfirewall firewall add rule name="Deskctl Bridge" dir=in action=allow program="$INSTDIR\Deskctl Bridge.exe" enable=yes'
!macroend
...
!macro NSIS_HOOK_PREUNINSTALL
    ...
    nsExec::ExecToLog 'netsh advfirewall firewall delete rule name="Deskctl Bridge"'
